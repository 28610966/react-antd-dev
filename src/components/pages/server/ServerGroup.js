...
const entityName = {name: I18nUtil.get('ServerGroup','服务组')};

class ServerGroup extends Component {
    ...
    loadGrid() {
        ReactUtil(this).action("ServerGroup.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'ServerGroup.get.data.id')) {
            reactUtil.setState({ ServerGroup: _.get(nextProps, 'ServerGroup.get')});
        }

        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `ServerGroup.${op}.data`;
            let errorId = `ServerGroup.${op}.error`;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (_.get(nextProps, 'ServerGroup.list.data', null) === null) {
            return false;
        }
        return true;
    }
    ...
    openNewModal = (flag) => {
        ...
        if (flag) {
            Object.assign(e, { ServerGroup: null});
        }else{
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ ServerGroup: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("ServerGroup.save",values);
                } else {
                    ReactUtil(this).action("ServerGroup.update",values);
                }
            }
        });
    }
    ...
    updateOne(e) {
        ReactUtil(this).action("ServerGroup.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("ServerGroup.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("ServerGroup.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'ServerGroup.get.loading', false)
            || _.get(this.props, 'ServerGroup.save.loading', false)
            || _.get(this.props, 'ServerGroup.update.loading', false);
    }
    ...
    renderNewModal() {
        const WrappedForm = Form.create()(ServerGroupForm);
        const data = _.get(this.state, 'ServerGroup', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'ServerGroup.list.data.list', null);
        const total = _.get(this.props, 'ServerGroup.list.data.total', 0);
        const loading = _.get(this.props, 'ServerGroup.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            prefix: '',
            forms: [
                        {id: 'groupName', label: '组名', type: 'input',
                        },
                        {id: 'order', label: '排序', type: 'number',
                        },
            ],
            loading: _.get(this.props, 'ServerGroup.list.loading', false),
            search: () => {
...
class ServerGroupForm extends Component {
    static propTypes = {
    ...
    render() {
        ...
        const fields = [
            ...
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '组名'}),
                    },
                ],
            },
            {
...
export default connect(mapStoreToProps)(ServerGroup);
