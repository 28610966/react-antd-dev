...
const entityName = {name: I18nUtil.get('User','人员')};
...
class User extends Component {
    ...
    componentWillMount() {
            ReactUtil(this).action("Org.list");
            ReactUtil(this).action("Role.list");
    }

    loadGrid() {
        ReactUtil(this).action("User.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'User.get.data.id')) {
            reactUtil.setState({ User: _.get(nextProps, 'User.get')});
        }
        if (reactUtil.diff(nextProps,'Org.list.data')) {
            let Orgs = _.get(nextProps, 'Org.list.data');
            reactUtil.setState({Dict: this.state.Dict.set("Orgs", _.map(Orgs, m => {
                return {value: m.id, label: m.name };
            }))});
        }
        if (reactUtil.diff(nextProps,'Role.list.data')) {
            let Roles = _.get(nextProps, 'Role.list.data');
            reactUtil.setState({Dict: this.state.Dict.set("Roles", _.map(Roles, m => {
                return {value: m.id, label: m.name };
            }))});
        }

        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `User.${op}.data`;
            let errorId = `User.${op}.error`;
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ User: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("User.save",values);
                } else {
                    ReactUtil(this).action("User.update",values);
                }
            }
        });
    }
        ...
        let columns = [
            ...
            },
            {
                ...
                width: '10%',
                 ...
                 render: (v, e) => {
                    let vv = v;
                       vv = _.chain(this.state.Dict.get("Orgs")).filter(s => v === s.value || _.indexOf(v,s.value) > -1).map(t => t.label).join(",").value();
                    return vv;
                }
            },
            {
            ...
            },
            {
                ...
                width: '10%',
                 ...
                 render: (v, e) => {
                    let vv = v;
                       vv = _.chain(this.state.Dict.get("Roles")).filter(s => v === s.value || _.indexOf(v,s.value) > -1).map(t => t.label).join(",").value();
                    return vv;
                }
            },
            {
    ...
    updateOne(e) {
        ReactUtil(this).action("User.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("User.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("User.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'User.get.loading', false)
            || _.get(this.props, 'User.save.loading', false)
            || _.get(this.props, 'User.update.loading', false);
    }
    ...
    renderNewModal() {
        ...
        const data = _.get(this.state, 'User', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'User.list.data.list', null);
        const total = _.get(this.props, 'User.list.data.total', 0);
        const loading = _.get(this.props, 'User.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            forms: [
                        ...
                        {id: 'en_name', label: '英文名', type: 'input',
                        },
                        {id: 'org_id', label: '组织', type: 'select',
                        option: this.state.Dict.get("Orgs"), 
                        },
                        {id: 'age', label: '年龄', type: 'number',
                        },
                        {id: 'right', label: '角色', type: 'checkbox',
                        option: this.state.Dict.get("Roles"), 
                        },
            ],
            loading: _.get(this.props, 'User.list.loading', false),
            search: () => {
    ...
    render() {
        ...
        const fields = [
            ...
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '姓名'}),
                    },
                ],
            },
            {
            ...
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '年龄'}),
                    },
                ],
            },
            {
                ...
                type: 'checkbox',
                span: 24,
                option: dict.get("Roles") , //字典
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '角色'}),
                    },
                ],
            },
        ]
