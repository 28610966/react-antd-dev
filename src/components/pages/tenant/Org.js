...
const entityName = {name: I18nUtil.get('Org','组织')};
...
class Org extends Component {
    ...
    loadGrid() {
        ReactUtil(this).action("Org.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'Org.get.data.id')) {
            reactUtil.setState({ Org: _.get(nextProps, 'Org.get')});
        }

        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `Org.${op}.data`;
            let errorId = `Org.${op}.error`;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (_.get(nextProps, 'Org.list.data', null) === null) {
            return false;
        }
        return true;
    }
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ Org: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("Org.save",values);
                } else {
                    ReactUtil(this).action("Org.update",values);
                }
            }
        });
    }
    ...
    updateOne(e) {
        ReactUtil(this).action("Org.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("Org.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("Org.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'Org.get.loading', false)
            || _.get(this.props, 'Org.save.loading', false)
            || _.get(this.props, 'Org.update.loading', false);
    }
    ...
    renderNewModal() {
        const WrappedForm = Form.create()(OrgForm);
        const data = _.get(this.state, 'Org', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'Org.list.data.list', null);
        const total = _.get(this.props, 'Org.list.data.total', 0);
        const loading = _.get(this.props, 'Org.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            prefix: '',
            forms: [
                        {id: 'name', label: '组织名', type: 'input',
                        },
            ],
            loading: _.get(this.props, 'Org.list.loading', false),
            search: () => {
    ...
    render() {
        ...
        const fields = [
            ...
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '组织名'}),
                    },
                ],
            },
        ]
...
export default connect(mapStoreToProps)(Org);
