...
const entityName = {name: I18nUtil.get('Tenant','租户')};
...
class Tenant extends Component {
    ...
    loadGrid() {
        ReactUtil(this).action("Tenant.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'Tenant.get.data.id')) {
            reactUtil.setState({ Tenant: _.get(nextProps, 'Tenant.get')});
        }
        ...
        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `Tenant.${op}.data`;
            let errorId = `Tenant.${op}.error`;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (_.get(nextProps, 'Tenant.list.data', null) === null) {
            return false;
        }
        return true;
    }
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ Tenant: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("Tenant.save",values);
                } else {
                    ReactUtil(this).action("Tenant.update",values);
                }
            }
        });
    }
    ...
    updateOne(e) {
        ReactUtil(this).action("Tenant.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("Tenant.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("Tenant.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'Tenant.get.loading', false)
            || _.get(this.props, 'Tenant.save.loading', false)
            || _.get(this.props, 'Tenant.update.loading', false);
    }
    ...
    renderNewModal() {
        const WrappedForm = Form.create()(TenantForm);
        const data = _.get(this.state, 'Tenant', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'Tenant.list.data.list', null);
        const total = _.get(this.props, 'Tenant.list.data.total', 0);
        const loading = _.get(this.props, 'Tenant.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            forms: [
                        ...
                        {id: 'type', label: '类别', type: 'select',
                        option: this.state.Dict.get("Dicts"), 
                        },
            ],
            loading: _.get(this.props, 'Tenant.list.loading', false),
            search: () => {
...
export default connect(mapStoreToProps)(Tenant);
