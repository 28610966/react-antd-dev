...
const entityName = {name: I18nUtil.get('Event','事件')};
...
class Event extends Component {
    ...
    loadGrid() {
        ReactUtil(this).action("Event.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'Event.get.data.id')) {
            reactUtil.setState({ Event: _.get(nextProps, 'Event.get')});
        }
        ...
        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `Event.${op}.data`;
            let errorId = `Event.${op}.error`;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (_.get(nextProps, 'Event.list.data', null) === null) {
            return false;
        }
        return true;
    }
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ Event: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("Event.save",values);
                } else {
                    ReactUtil(this).action("Event.update",values);
                }
            }
        });
    }
    ...
    updateOne(e) {
        ReactUtil(this).action("Event.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("Event.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("Event.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'Event.get.loading', false)
            || _.get(this.props, 'Event.save.loading', false)
            || _.get(this.props, 'Event.update.loading', false);
    }
    ...
    renderNewModal() {
        const WrappedForm = Form.create()(EventForm);
        const data = _.get(this.state, 'Event', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'Event.list.data.list', null);
        const total = _.get(this.props, 'Event.list.data.total', 0);
        const loading = _.get(this.props, 'Event.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            prefix: '',
            forms: [
                        {id: 'name', label: '事件名', type: 'input',
                        },
                        {id: 'date', label: '发生时间', type: 'datepicker',
                        span: 8, 
                        },
                        {id: 'source', label: '事件源', type: 'select',
                        option: this.state.Dict.get("Dicts"), 
                        },
                        {id: 'handleUser', label: '处理人', type: 'input',
                        },
            ],
            loading: _.get(this.props, 'Event.list.loading', false),
            search: () => {
    ...
    render() {
        ...
        const fields = [
            ...
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '事件名'}),
                    },
                ],
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '发生时间'}),
                    },
                ],
            },
            {
            ...
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '处理人'}),
                    },
                ],
            },
        ]
...
export default connect(mapStoreToProps)(Event);
