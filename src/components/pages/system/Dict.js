...
const entityName = {name: I18nUtil.get('Dict','字典')};
...
class Dict extends Component {
    ...
    loadGrid() {
        ReactUtil(this).action("Dict.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'Dict.get.data.id')) {
            reactUtil.setState({ Dict: _.get(nextProps, 'Dict.get')});
        }

        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `Dict.${op}.data`;
            let errorId = `Dict.${op}.error`;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (_.get(nextProps, 'Dict.list.data', null) === null) {
            return false;
        }
        return true;
    }
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ Dict: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("Dict.save",values);
                } else {
                    ReactUtil(this).action("Dict.update",values);
                }
            }
        });
    }
    ...
    updateOne(e) {
        ReactUtil(this).action("Dict.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("Dict.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("Dict.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'Dict.get.loading', false)
            || _.get(this.props, 'Dict.save.loading', false)
            || _.get(this.props, 'Dict.update.loading', false);
    }
    ...
    renderNewModal() {
        const WrappedForm = Form.create()(DictForm);
        const data = _.get(this.state, 'Dict', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'Dict.list.data.list', null);
        const total = _.get(this.props, 'Dict.list.data.total', 0);
        const loading = _.get(this.props, 'Dict.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            loading: _.get(this.props, 'Dict.list.loading', false),
            search: () => {
...
export default connect(mapStoreToProps)(Dict);
