...
const entityName = {name: I18nUtil.get('ExternalPage','外部页面')};
...
class ExternalPage extends Component {
    ...
    loadGrid() {
        ReactUtil(this).action("ExternalPage.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'ExternalPage.get.data.id')) {
            reactUtil.setState({ ExternalPage: _.get(nextProps, 'ExternalPage.get')});
        }

        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `ExternalPage.${op}.data`;
            let errorId = `ExternalPage.${op}.error`;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (_.get(nextProps, 'ExternalPage.list.data', null) === null) {
            return false;
        }
        return true;
    }
    ...
    openNewModal = (flag) => {
        ...
        if (flag) {
            Object.assign(e, { ExternalPage: null});
        }else{
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ ExternalPage: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("ExternalPage.save",values);
                } else {
                    ReactUtil(this).action("ExternalPage.update",values);
                }
            }
        });
    }
    ...
    updateOne(e) {
        ReactUtil(this).action("ExternalPage.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("ExternalPage.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("ExternalPage.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'ExternalPage.get.loading', false)
            || _.get(this.props, 'ExternalPage.save.loading', false)
            || _.get(this.props, 'ExternalPage.update.loading', false);
    }
    ...
    renderNewModal() {
        const WrappedForm = Form.create()(ExternalPageForm);
        const data = _.get(this.state, 'ExternalPage', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'ExternalPage.list.data.list', null);
        const total = _.get(this.props, 'ExternalPage.list.data.total', 0);
        const loading = _.get(this.props, 'ExternalPage.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            forms: [
                        ...
                        {id: 'url', label: '地址', type: 'input',
                        }
            ],
            loading: _.get(this.props, 'ExternalPage.list.loading', false),
            search: () => {
...
class ExternalPageForm extends Component {
    static propTypes = {
    ...
    render() {
        ...
        const fields = [
            ...
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '名称'}),
                    },
                ],
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '地址'}),
                    },
                ],
            },
            {
...
export default connect(mapStoreToProps)(ExternalPage);
