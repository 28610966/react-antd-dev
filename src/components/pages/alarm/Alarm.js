...
const entityName = {name: I18nUtil.get('Alarm','告警')};
...
class Alarm extends Component {
    ...
    loadGrid() {
        ReactUtil(this).action("Alarm.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'Alarm.get.data.id')) {
            reactUtil.setState({ Alarm: _.get(nextProps, 'Alarm.get')});
        }
        ...
        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `Alarm.${op}.data`;
            let errorId = `Alarm.${op}.error`;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (_.get(nextProps, 'Alarm.list.data', null) === null) {
            return false;
        }
        return true;
    }
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ Alarm: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("Alarm.save",values);
                } else {
                    ReactUtil(this).action("Alarm.update",values);
                }
            }
        });
    }
    ...
    updateOne(e) {
        ReactUtil(this).action("Alarm.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("Alarm.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("Alarm.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'Alarm.get.loading', false)
            || _.get(this.props, 'Alarm.save.loading', false)
            || _.get(this.props, 'Alarm.update.loading', false);
    }
    ...
    renderNewModal() {
        ...
        const data = _.get(this.state, 'Alarm', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'Alarm.list.data.list', null);
        const total = _.get(this.props, 'Alarm.list.data.total', 0);
        const loading = _.get(this.props, 'Alarm.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            prefix: '',
            forms: [
                        {id: 'level', label: '级别', type: 'select',
                        option: this.state.Dict.get("Dicts"), 
                        },
                        {id: 'alarm_time', label: '告警事件', type: 'datepicker',
                        span: 8, 
                        },
            ],
            loading: _.get(this.props, 'Alarm.list.loading', false),
            search: () => {
...
export default connect(mapStoreToProps)(Alarm);
