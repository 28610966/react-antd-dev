...
const entityName = {name: I18nUtil.get('Accisdent','事故')};

class Accisdent extends Component {
    ...
    loadGrid() {
        ReactUtil(this).action("Accisdent.list",this.state.query.toObject());
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (reactUtil.diff(nextProps,'Accisdent.get.data.id')) {
            reactUtil.setState({ Accisdent: _.get(nextProps, 'Accisdent.get')});
        }

        ['save', 'update', 'delete'].forEach((op) => {
            let dataId = `Accisdent.${op}.data`;
            let errorId = `Accisdent.${op}.error`;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (_.get(nextProps, 'Accisdent.list.data', null) === null) {
            return false;
        }
        return true;
    }
    ...
    openNewModal = (flag) => {
        ...
        if (flag) {
            Object.assign(e, { Accisdent: null});
        }else{
    ...
    handleOk() {
        this.refs.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                this.setState({ Accisdent: {data: values}});
                if (_.get(values, 'id', null) === null) {
                    ReactUtil(this).action("Accisdent.save",values);
                } else {
                    ReactUtil(this).action("Accisdent.update",values);
                }
            }
        });
    }
        ...
        let columns = [
            ...
            },
            {
                ...
                width: '10%',
                 ...
                 render: (v, e) => {
                    let vv = v;
                       vv = _.chain(this.state.Dict.get("s")).filter(s => v === s.value || _.indexOf(v,s.value) > -1).map(t => t.label).join(",").value();
                    return vv;
                }
            },
            {
    ...
    updateOne(e) {
        ReactUtil(this).action("Accisdent.get",e);
    ...
    deleteOne(e) {
        ReactUtil(this).action("Accisdent.delete",e);
    }
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    var ids = this.state.selectedRows.map(m => m.id).join(',');
                     ReactUtil(this).action("Accisdent.delete",{id: ids});
                }
            });
        }
    }
    ...
    getLoading() {
        return _.get(this.props, 'Accisdent.get.loading', false)
            || _.get(this.props, 'Accisdent.save.loading', false)
            || _.get(this.props, 'Accisdent.update.loading', false);
    }
    ...
    renderNewModal() {
        const WrappedForm = Form.create()(AccisdentForm);
        const data = _.get(this.state, 'Accisdent', null);
    ...
    renderTable() {
        ...
        const list = _.get(this.props, 'Accisdent.list.data.list', null);
        const total = _.get(this.props, 'Accisdent.list.data.total', 0);
        const loading = _.get(this.props, 'Accisdent.list.loading', false);
        return (
            <div>
                {this.renderTools()}
    ...
    renderSearch() {
        var props = {
            ...
            prefix: '',
            forms: [
                        {id: 'name', label: '事故名', type: 'input',
                        },
                        {id: 'type', label: '事故类型', type: 'select',
                        option: this.state.Dict.get("s"), 
                        },
            ],
            loading: _.get(this.props, 'Accisdent.list.loading', false),
            search: () => {
    ...
    render() {
        return (
            <div className="Accisdent_container">
                {this.renderSearch()}
                {this.renderTable()}
                {this.renderNewModal()}
            </div>);
...
class AccisdentForm extends Component {
    static propTypes = {
    ...
    render() {
        ...
        const fields = [
            ...
            },
            {
                ...
                rules: [
                    {
                    required:true, message: I18nUtil.get('please.input', {name: '事故名'}),
                    },
                ],
            },
            {
...
function mapStoreToProps(store) {
    return {
        Accisdent: store.get('Accisdent'),
    }
}

export default connect(mapStoreToProps)(Accisdent);
